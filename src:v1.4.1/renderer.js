...
async function init() {
    ...
    if (platform === 'darwin') {
        ...
        const playitToggle = document.getElementById('setup-playit-toggle');
        if (macWarning) macWarning.style.display = 'block';
        if (playitToggle) {
            playitToggle.disabled = true;
            playitToggle.checked = false;
        }
        const settingsToggle = document.getElementById('settings-playit-enabled');
...
async function updateNetworkInfo() {
    ...
    if (currentConfig.playitEnabled && currentConfig.playitAddress) {
        document.getElementById('header-ip').innerText = currentConfig.playitAddress;
        document.getElementById('header-port').innerText = "Default";
    } else {
...
async function initSoftwareList() {
    ...
    try {
        ...
        if (projects && projects.length > 0) {
            ...
            await window.updateSoftwareVersions();
        } else {
...
async function loadSettingsPage() {
    ...
    if (slider) {
        slider.value = currentConfig.ram || '4';
        document.getElementById('ram-disp').innerText = slider.value + ' GB';
    ...
    const discordToggle = document.getElementById('discord-enabled');
    const inviteArea = document.getElementById('discord-invite-area');
    const inviteLink = document.getElementById('discord-invite-link');

    if (discordTokenInput) discordTokenInput.value = currentConfig.discordToken || '';
    if (discordToggle) discordToggle.checked = currentConfig.discordEnabled || false;
    
    if (currentConfig.discordToken) {
        const link = await window.api.getDiscordLink(currentConfig.discordToken);
        if (link && inviteArea && inviteLink) {
    ...
    const iconImg = document.getElementById('motd-icon');
    if (iconImg) {
        iconImg.src = iconData ? iconData : 'https://minecraft.net/favicon.png';
    }
...
async function refreshServerIcon() {
    try {
        ...
        if (iconDataUrl) {
            img.src = iconDataUrl + '?t=' + Date.now();  
        } else {
...
document.querySelector('#motd-preview > div[style*="position: relative"]').onclick = async () => {
    try {
        ...
        if (result.success) {
            ...
            setTimeout(refreshServerIcon, 600); 
        } else {
...
if(saveBtn) {
    saveBtn.onclick = async () => {
        const playitEnabled = document.getElementById('settings-playit-enabled').checked;
        const playitAddr = document.getElementById('settings-playit-address').value;
        await window.api.savePlayitSettings(playitEnabled, playitAddr);
        ...
        try {
            ...
            await window.api.saveDiscordSettings(discordToken, discordEnabled);
            
            if (discordToken) {
                const link = await window.api.getDiscordLink(discordToken);
                if (link) {
                    document.getElementById('discord-invite-area').style.display = 'block';
                    document.getElementById('discord-invite-link').href = link;
                }
            }
            ...
            properties['view-distance'] = document.getElementById('prop-view-distance').value;
            ...
            alert("All settings saved and Discord bot updated!");

        } catch (err) {
            console.error(err);
            alert("Save failed. Check terminal for details.");
        }
