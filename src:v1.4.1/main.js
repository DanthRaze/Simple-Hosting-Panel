...
function parseInterval(val) {
    ...
    if (val.includes('m')) return num * 30 * 24 * 60 * 60 * 1000; 
    return 0;
}
...
ipcMain.handle('save-playit-settings', async (event, enabled, address) => {
    try {
        serverConfig.playitEnabled = enabled;
        serverConfig.playitAddress = address;

        saveConfig();

        return { success: true };
    } catch (err) {
        console.error("Failed to save playit settings:", err);
...
function startMinecraftServer() {
    ...
    if (serverConfig.playitEnabled && process.platform !== 'darwin') {
        const playitPath = path.join(serverConfig.folderPath, process.platform === 'win32' ? 'playit.exe' : 'playit');
        if (fs.existsSync(playitPath)) {
            playitProcess = spawn(playitPath, [], { cwd: serverConfig.folderPath });
            playitProcess.stdout.on('data', (data) => console.log(`[Playit] ${data}`));
        } else {
            console.error("Playit binary not found in server folder!");
        }
    }
